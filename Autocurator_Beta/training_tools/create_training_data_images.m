...
function create_training_data_images(tArray, cArray, sizeROI, vidPath, transferDir, metaIter)
  bitSize = 8;
  maxRamGB = 1;
  % Derived variables
  framesPerSet = 1024;
  ...
  for i = 1:200
    ...
    for j = poleStartTime:poleStopTime
      ...
      xPole = round(tArray.trials{i}.whiskerTrial.barPos(j,2));
      yPole = round(tArray.trials{i}.whiskerTrial.barPos(j,3));
...
%         newY = ceil(mean(newY));
        ...
        cFileName = sprintf('%s_%04d.png',videoName,j);
        cFullFileName = ['C:\SuperUser\CNN_Projects\1000_Frame_Model\Touches_JK2' filesep cFileName];
      ...
      elseif sum(ismember([j-20:j+20], cArray{i}.contactInds{1})) > 0
        ...
        % Save Frame
        cFileName = sprintf('%s_%04d.png',videoName,j);
        cFullFileName = ['C:\SuperUser\CNN_Projects\1000_Frame_Model\NonTouches_JK2' filesep cFileName];
        imwrite(nFrameMat, cFullFileName, 'png');
        % Stuff
      end
    end
